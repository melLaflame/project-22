...
Chaines* generationAleatoire(int nbChaines,int nbPointsChaine,int xmax,int ymax){
	double x;
	double y;
	CellChaine* cell_chaine;
	CellPoint* cell_point;
	Chaines* Chaine = creer_Chaines(nbChaines,0);

	for(int i=1;i<=nbChaines;i++){
		cell_chaine = creer_CellChaine(i);
		ajout_en_tete_CellChaine(Chaine,cell_chaine);

		for(int j=1;j<=nbPointsChaine;j++){
			x = (rand()/(double)RAND_MAX) * (xmax);
			y = (rand()/(double)RAND_MAX) * (ymax);
			cell_point = creer_CellPoint(x,y);
			ajout_en_tete_CellPoint(cell_chaine,cell_point);
		}
	}
	return Chaine;
}
